...
TConclusion<bool> TPortionDataSource::DoStartReserveMemory(const NArrow::NSSA::TProcessorContext& context,
    ...
    auto allocation = std::make_shared<NCommon::TAllocateMemoryStep::TFetchingStepAllocation>(source, sizeToReserve,
        GetExecutionContext().GetCursorStep(), policy->GetStage(), false /*needNextStep*/,
        IsScanMemoryLimiterEnabled(GetContext()
                                       ->GetCommonContext()
                                       ->GetReadMetadataPtrVerifiedAs<NCommon::TReadMetadata>()
                                       ->GetGroupedMemoryLimiterOperator()) /*scheduleContinuation*/);
